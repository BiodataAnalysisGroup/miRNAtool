...
RTC.n<- rowMeans(as.matrix(t(RTC[,c(2:4)])))
RTC.c<- rowMeans(as.matrix(t(RTC[,c(5:7)])))
RTC.all<-matrix(0, nrow = 3, ncol = 2)

RTC.all[,1]<- t(RTC.n)
RTC.all[,2]<- t(RTC.c)
RTC.all<- as.data.frame(RTC.all)
